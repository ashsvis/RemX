...
type
  TKontFDEditForm = class(TBaseEditForm)
    ListView1: TListView;
    ListView2: TListView;
    Splitter1: TSplitter;
...
var
  KontFDEditForm: TKontFDEditForm;
...
procedure TKontFDEditForm.BaseFresh(var Mess: TMessage);
begin
  UpdateRealTime;
end;

procedure TKontFDEditForm.ConnectBaseEditor(var Mess: TMessage);
begin
  ConnectImageList(TImageList(Mess.WParam));
  ConnectEntity(TEntity(Mess.LParam));
end;

procedure TKontFDEditForm.ConnectEntity(Entity: TEntity);
var i: integer;
begin
  E:=Entity as TKontFDGroup;
  with ListView1 do
  begin
    Items.BeginUpdate;
    try
...
procedure TKontFDEditForm.ConnectImageList(ImageList: TImageList);
begin
  ListView1.SmallImages:=ImageList;
  ListView2.SmallImages:=ImageList;
end;

procedure TKontFDEditForm.ListView2DblClick(Sender: TObject);
var E: TEntity; 
begin
...
procedure TKontFDEditForm.ListView1DblClick(Sender: TObject);
begin
...
procedure TKontFDEditForm.UpdateRealTime;
var L: TListItem;
begin
...
procedure TKontFDEditForm.PopupMenu1Popup(Sender: TObject);
...
procedure TKontFDEditForm.ChangeTextClick(Sender: TObject);
...
procedure TKontFDEditForm.ChangeFetchClick(Sender: TObject);
...
procedure TKontFDEditForm.ChangeBooleanClick(Sender: TObject);
...
procedure TKontFDEditForm.PopupMenu2Popup(Sender: TObject);
...
procedure TKontFDEditForm.ChangeLinkClick(Sender: TObject);
...
begin
  ...
  try
    ...
    begin
      ...
      while R <> nil do
      begin
        if not Assigned(R.SourceEntity) and R.IsDigit and not R.IsParam and
          (not R.IsVirtual and (E.Channel = R.Channel) and (E.Node = R.Node) or
           R.IsVirtual) then
          List.Add(R);
        R:=R.NextEntity;
      end;
...
procedure TKontFDEditForm.DeleteLinkClick(Sender: TObject);
...
procedure TKontFDEditForm.ChangeIntegerClick(Sender: TObject);
