...
type
  TKontGOPaspForm = class(TForm,IEntity)
    Panel1: TPanel;
    PTNameDesc: TLabel;
    PTDesc: TLabel;
...
var
  KontGOPaspForm: TKontGOPaspForm;
...
procedure TKontGOPaspForm.ConnectEntity(Entity: TEntity);
var i,YPos,XPos: integer; L: TLabel; P: TShape;
begin
  E:=Entity as TKontOutputsGroup;
...
//---------------------------------
      ...
      begin
        ...
        begin
          ...
          if E.EntityChilds[i] is TKontDigOut then
            Caption:=(E.EntityChilds[i] as TKontDigOut).PtText
          else
          if E.EntityChilds[i] is TKontAnaOut then
            Caption:=(E.EntityChilds[i] as TKontAnaOut).PtText
          else
            Caption:='??????';
          OnMouseEnter:=AlarmOnMouseEnter;
          OnMouseLeave:=AlarmOnMouseLeave;
...
procedure TKontGOPaspForm.ConnectImageList(ImageList: TImageList);
begin
// Stub
end;

procedure TKontGOPaspForm.UpdateRealTime;
...
procedure TKontGOPaspForm.FormCreate(Sender: TObject);
begin
  List:=TList.Create;
  ObjList:=TComponentList.Create(True);
end;

procedure TKontGOPaspForm.FormResize(Sender: TObject);
begin
...
procedure TKontGOPaspForm.AlarmOnMouseDown(Sender: TObject;
...
procedure TKontGOPaspForm.AlarmOnMouseEnter(Sender: TObject);
begin
...
procedure TKontGOPaspForm.AlarmOnMouseLeave(Sender: TObject);
begin
  with Sender as TLabel do
    TShape(Tag).Pen.Color:=clBlack;
end;

procedure TKontGOPaspForm.ButtonToDataBaseClick(Sender: TObject);
begin
  E.ShowEditor(Monitor.MonitorNum);
end;

procedure TKontGOPaspForm.FormDestroy(Sender: TObject);
begin
  ObjList.Free;
  List.Free;
end;

procedure TKontGOPaspForm.LabelDblClick(Sender: TObject);
var L: TLabel; P: TShape; T: TEntity;
begin
  L:=TLabel(Sender);
  if L is TLabel then
...
procedure TKontGOPaspForm.ButtonToMainScreenClick(Sender: TObject);
begin
