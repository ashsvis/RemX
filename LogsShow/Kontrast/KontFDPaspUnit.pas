...
type
  TKontFDPaspForm = class(TForm,IEntity)
    Panel1: TPanel;
    PTNameDesc: TLabel;
    PTDesc: TLabel;
...
var
  KontFDPaspForm: TKontFDPaspForm;
...
procedure TKontFDPaspForm.ConnectEntity(Entity: TEntity);
...
procedure TKontFDPaspForm.ConnectImageList(ImageList: TImageList);
begin
// Stub
end;

procedure TKontFDPaspForm.UpdateRealTime;
...
procedure TKontFDPaspForm.FormCreate(Sender: TObject);
begin
  List:=TList.Create;
...
procedure TKontFDPaspForm.FormResize(Sender: TObject);
begin
...
procedure TKontFDPaspForm.AlarmOnMouseDown(Sender: TObject;
...
procedure TKontFDPaspForm.AlarmOnMouseEnter(Sender: TObject);
begin
...
procedure TKontFDPaspForm.AlarmOnMouseLeave(Sender: TObject);
begin
  with Sender as TLabel do
    TShape(Tag).Pen.Color:=clBlack;
end;

procedure TKontFDPaspForm.ButtonToDataBaseClick(Sender: TObject);
begin
  E.ShowEditor(Monitor.MonitorNum);
end;

procedure TKontFDPaspForm.FormDestroy(Sender: TObject);
begin
  ObjList.Free;
  List.Free;
  ListVal.Free;
...
procedure TKontFDPaspForm.LabelDblClick(Sender: TObject);
var L: TLabel; P: TShape; T: TEntity;
begin
  L:=TLabel(Sender);
  if L is TLabel then
...
procedure TKontFDPaspForm.ButtonToMainScreenClick(Sender: TObject);
begin
