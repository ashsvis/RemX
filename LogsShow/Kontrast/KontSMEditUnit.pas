...
type
  TKontSMEditForm = class(TBaseEditForm)
    ListView1: TListView;
  ...
  private
    ...
    procedure ChangeActionKindClick(Sender: TObject);
  public
  end;

var
  KontSMEditForm: TKontSMEditForm;
...
procedure TKontSMEditForm.BaseFresh(var Mess: TMessage);
begin
  UpdateRealTime;
end;

procedure TKontSMEditForm.ConnectBaseEditor(var Mess: TMessage);
begin
  ConnectImageList(TImageList(Mess.WParam));
  ConnectEntity(TEntity(Mess.LParam));
end;

procedure TKontSMEditForm.ConnectEntity(Entity: TEntity);
begin
  E:=Entity as TKontSyncMagister;
  with ListView1 do
  begin
    Items.BeginUpdate;
    try
      ...
      begin
        ImageIndex:=-1;
        Caption:='Тип синхронизации';
        SubItems.Add(ASyncKind[E.ActionKind]);
      end;
    finally
      Items.EndUpdate;
    end;
...
procedure TKontSMEditForm.ConnectImageList(ImageList: TImageList);
begin
  ListView1.SmallImages:=ImageList;
end;

procedure TKontSMEditForm.UpdateRealTime;
var L: TListItem;
begin
...
procedure TKontSMEditForm.PopupMenu1Popup(Sender: TObject);
...
begin
  ...
  begin
    ...
    1,2: begin
      ...
      7: for i:=Ord(Low(ASyncKind)) to Ord(High(ASyncKind)) do
         begin
           M:=TMenuItem.Create(Self);
           M.Caption:=ASyncKind[TSyncKind(i)];
           M.Checked:=(E.ActionKind=TSyncKind(i));
           M.Tag:=i;
           M.OnClick:=ChangeActionKindClick;
           Items.Add(M);
         end;
   end;
  end;
...
procedure TKontSMEditForm.ChangeActionKindClick(Sender: TObject);
var M: TMenuItem; L: TListItem;
begin
  L:=ListView1.Selected;
  M:=Sender as TMenuItem;
  E.ActionKind:=TSyncKind(M.Tag);
  L.SubItems[0]:=ASyncKind[E.ActionKind];
end;

procedure TKontSMEditForm.ChangeIntegerClick(Sender: TObject);
...
procedure TKontSMEditForm.ChangeTextClick(Sender: TObject);
...
procedure TKontSMEditForm.ChangeBooleanClick(Sender: TObject);
...
procedure TKontSMEditForm.ChangeFetchClick(Sender: TObject);
