...
type
  TModbusAMEditForm = class(TBaseEditForm)
    ListView1: TListView;
    ListView2: TListView;
    Splitter1: TSplitter;
...
var
  ModbusAMEditForm: TModbusAMEditForm;
...
procedure TModbusAMEditForm.BaseFresh(var Mess: TMessage);
begin
  UpdateRealTime;
end;

procedure TModbusAMEditForm.ConnectBaseEditor(var Mess: TMessage);
begin
  ConnectImageList(TImageList(Mess.WParam));
  ConnectEntity(TEntity(Mess.LParam));
end;

procedure TModbusAMEditForm.ConnectEntity(Entity: TEntity);
var i: integer;
begin
  E:=Entity as TModbusAMGroup;
  with ListView1 do
  begin
    Items.BeginUpdate;
    try
...
procedure TModbusAMEditForm.ConnectImageList(ImageList: TImageList);
begin
  ListView1.SmallImages:=ImageList;
  ListView2.SmallImages:=ImageList;
end;

procedure TModbusAMEditForm.ListView2DblClick(Sender: TObject);
var E: TEntity;
begin
...
procedure TModbusAMEditForm.ListView1DblClick(Sender: TObject);
begin
...
procedure TModbusAMEditForm.UpdateRealTime;
var L: TListItem;
begin
...
procedure TModbusAMEditForm.PopupMenu1Popup(Sender: TObject);
var L: TListItem; M: TMenuItem; u: TAddrPrefix; df: TModbusFormat;
begin
...
procedure TModbusAMEditForm.ChangeTextClick(Sender: TObject);
...
procedure TModbusAMEditForm.ChangeFetchClick(Sender: TObject);
...
procedure TModbusAMEditForm.ChangeBooleanClick(Sender: TObject);
...
procedure TModbusAMEditForm.PopupMenu2Popup(Sender: TObject);
...
procedure TModbusAMEditForm.ChangeLinkClick(Sender: TObject);
...
begin
  ...
  try
    ...
    begin
      ...
      while R <> nil do
      begin
        IsModbus:=(R.ClassType = TModbusAnaOut);
        if not Assigned(R.SourceEntity) and
           R.IsAnalog and not R.IsVirtual and
          (E.Channel = R.Channel) and (E.Node = R.Node) and IsModbus and
          (E.DataFormat = TModbusAnaOut(R).DataFormat)
          or
...
procedure TModbusAMEditForm.DeleteLinkClick(Sender: TObject);
...
procedure TModbusAMEditForm.ChangeIntegerClick(Sender: TObject);
  ...
  begin
    ...
    begin
      case L.Index of
        ...
        6: if InRange(V,1,125) then
           begin
             E.DataCount:=V;
             L.SubItems[0]:=Format('%d',[E.DataCount]);
           end
           else
             raise ERangeError.Create('Значение вне допустимых границ (1..125)!');
      end;
      if L.Index in [1..2,6] then
      begin
        UpdateBaseView(Self);
        RestoreEntityPos(E);
      end;
...
procedure TModbusAMEditForm.ChangeAddrPrefixClick(Sender: TObject);
...
procedure TModbusAMEditForm.ChangeDataFormatClick(Sender: TObject);
var L: TListItem; M: TMenuItem;
begin
  L:=ListView1.Selected;
  M:=Sender as TMenuItem;
