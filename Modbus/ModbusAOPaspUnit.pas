unit ModbusAOPaspUnit;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, StdCtrls, ExtCtrls, Buttons, EntityUnit, ModbusUnit, RxGraph;

type
  TModbusAOPaspForm = class(TForm,IEntity)
    Panel1: TPanel;
    PTNameDesc: TLabel;
    PTDesc: TLabel;
...
var
  ModbusAOPaspForm: TModbusAOPaspForm;
...
procedure TModbusAOPaspForm.ConnectEntity(Entity: TEntity);
...
begin
  ...
  DataFormat.Caption:=ADataFormat[E.DataFormat];
...
//-------------------------------------
  ...
  Address.Caption:=Format('%s, %5.5d',
                         [Copy(AAnaAddrPrefix[E.AddrPrefix],1,6),E.Address]);
  UpdateRealTime;
end;

procedure TModbusAOPaspForm.ConnectImageList(ImageList: TImageList);
begin
// Stub
end;

procedure TModbusAOPaspForm.UpdateRealTime;
...
procedure TModbusAOPaspForm.FormCreate(Sender: TObject);
begin
...
procedure TModbusAOPaspForm.FormResize(Sender: TObject);
begin
  RxGraph.Left:=10;
...
procedure TModbusAOPaspForm.PVHHTPMouseDown(Sender: TObject;
...
procedure TModbusAOPaspForm.PVHHTPMouseEnter(Sender: TObject);
begin
...
procedure TModbusAOPaspForm.PVHHTPMouseLeave(Sender: TObject);
begin
  with Sender as TLabel do
    TShape(Tag).Pen.Color:=clBlack;
end;

procedure TModbusAOPaspForm.ButtonToDataBaseClick(Sender: TObject);
begin
  E.ShowEditor(Monitor.MonitorNum);
end;

procedure TModbusAOPaspForm.PVSourceDblClick(Sender: TObject);
begin
  E.ShowParentPassport(Monitor.MonitorNum);
end;

procedure TModbusAOPaspForm.ButtonToMainScreenClick(Sender: TObject);
begin
