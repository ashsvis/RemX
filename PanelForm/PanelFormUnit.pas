...
begin
  ...
  if RemXForm.Registered then
    CaptionLabel.Caption:='Автоматизированная система контроля и управления'
  else
    CaptionLabel.Caption:='Автоматизированная система контроля и управления'+
...
begin
  ...
  try
    ...
    if ShowUsersForm.ShowModal = mrAbort then // отмена регистрации
    begin
      RemXForm.LongUserName := '';
      Caddy.UserLevel := 0;
      UserPanel.Caption := 'Нет регистрации';
      Caddy.AddChange('Система', 'Регистрация', RemXForm.ShortUserName,'',
                      'Отмена регистрации', RemXForm.ShortUserName);
...
begin
  case Caddy.NetRole of
   nrServer: S := Format('Станция №%d, сервер, ', [Caddy.Station]);
   nrClient: S := Format('Станция №%d, клиент, ', [Caddy.Station]);
  else
   S := Format('Станция №%d, ', [Caddy.Station]);
  end;
...
begin
  ...
  CaptionLabel.Caption:='Просмотр актуальных аварийных сообщений';
end;
...
begin
  ...
  CaptionLabel.Caption:='Просмотр актуальных переключений';
end;
...
begin
  ...
  CaptionLabel.Caption:='Редактор базы данных';
  actBaseEdit.Tag:=0;
end;
...
begin
  ...
  CaptionLabel.Caption:='Просмотр графиков';
end;
...
begin
  ...
  CaptionLabel.Caption := 'Просмотр таблиц';
end;
...
begin
  ...
  CaptionLabel.Caption := 'Просмотр отчетов';
end;
...
begin
  ...
  CaptionLabel.Caption := 'Архив отчетов';
end;
...
begin
  ...
  CaptionLabel.Caption := 'Конфигуратор исторических групп параметров';
end;
...
begin
  ...
  EditSchemeForm.Show;
  CaptionLabel.Caption := 'Редактор мнемосхем';
end;
...
begin
  ...
  CaptionLabel.Caption := 'Редактор отчетов';
end;
...
begin
  ...
  try
    ...
    if Authorization(KeyWord) then
      Caption := 'Владелец копии: ' + Config.ReadString('UserInfo','Name','');
    ShowModal;
  finally
    Release;
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      if RemXForm.Registered then
      begin
        RemXForm.ShowInfo('Активация прошла успешно!');
        CaptionLabel.Caption := 'Автоматизировання система контроля и управления';
        Update;
        Close;
      end
      else
        RemXForm.ShowError('Введённый код активации неверный!');
    end;
  finally
    Release;
  end;
...
begin
  ...
  CaptionLabel.Caption := 'Предварительный просмотр';
end;

procedure TPanelForm.RealTrendDataRefresh;
var i: integer;
begin
//=== Освежение данных в формах реальных трендов мнемосхем ===
  for i:=0 to RealTrendList.Count-1 do
...
begin
  if (SecondOfTheHour(Now) mod 3) = 0 then
    RealTrendDataRefresh; // Заполнение данными плавающих трендов
end;
...
begin
  ...
  begin
    Status:='Нет установленных принтеров в системе.';
    Result:=False;
    Exit;
  end;
  try
    DefaultName:=Printer.Printers[Printer.PrinterIndex];
    S:='Принтер: '+DefaultName+PrinterStatus(DefaultName)+' '+
                  JobStatus(DefaultName,JobExist);
    if JobExist then Status:=S;
    Result:=not JobExist;
  except
    Status:='Не выбран принтер по умолчанию.';
    Result:=False;
    Exit;
  end;
end;
...
//  Printer.PrinterIndex:=-1;
  try
    ...
    if PrinterBusy and
       (RemXForm.ShowQuestion('Вы действительно хотите удалить все документы для "'+
        sName+'"?') = mrOk) then
    begin
      ClearJobs(sName);
    end;
  except
    RemXForm.ShowMessage:='Не выбран принтер по умолчанию.';
  end;
end;
...
begin
  RemXForm.ShowMessage:='Нет сообщений';
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              RemXForm.ShowError('Запрашиваемый файл не найден по указанному пути!');
          end
          else
            RemXForm.ShowError('Не достаточно полномочий для запуска этой программы!');
          Break;
        end;
        N:=N.getNextSibling;
        Inc(i);
...
(*
  ...
  if Assigned(EditSchemeForm) and
    ...
    (RemXForm.ShowQuestion('Редактор мнемосхем содержит '+
                  'несохраненную схему. Записать?') = mrOk) then
  ...
  if Assigned(EditReportForm) and
    ...
    (RemXForm.ShowQuestion('Редактор отчетов содержит '+
                  'несохраненный отчет. Записать?') = mrOk) then
  begin
