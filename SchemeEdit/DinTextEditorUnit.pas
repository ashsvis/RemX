    ...
    begin
      ...
      begin
        if State then
          ...
          CurrColor:=Color0;
        //if NoLink then CurrColor:=clBlue;  // коммент 08.10.14
        if AlarmFound then
        begin
          if HasConfirm then
...
// Рамки выбора
    ...
    if not Editing and IsLinked and NoLink then     // добавлено 08.10.14
    begin                                           // добавлено 08.10.14
      Cash.Brush.Color:=clBlue;                     // добавлено 08.10.14
      Cash.FrameRect(ClientRect);                   // добавлено 08.10.14
    end;                                            // добавлено 08.10.14
    if not Editing and Focused then
    begin
      Cash.Brush.Color:=clAqua;
...
begin
  ...
  begin
    if Editing then
      Hint:='Динамический текст: <'+E.PtName+'>'+#13+
        Format('Смещение: %d, %d; Размер: %d, %d',[Left,Top,Width,Height])
    else
