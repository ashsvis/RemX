...
begin
  ...
  begin
    ...
    ReleaseCapture;
    DrawCurrRect;
    if Background.Cursor = crDrag then  // Добавлене нового динэлемента
    begin
      Found:=False;
      for i:=0 to DinActionList.ActionCount-1 do
    ...
    begin // Выбор посредством прямоугольника
      for i:=0 to DinList.Count-1 do
      begin
...
begin
  RemXForm.ShowMessage:=
                Format('Смещение: %3d, %3d; Размер: %3d, %3d; Выбрано: %d',
...
begin
  ...
  if RemxForm.ShowQuestion(
        NumToStr(n,'Выделен','','о','о',False)+' '+
        NumToStr(n,'объект','','а','ов')+' - Удалить?') = mrOk then
  begin
    ...
    Curr:=nil;
    RemXForm.ShowMessage:=
               NumToStr(n,'Удален','','о','о',False)+' '+
               NumToStr(n,'объект','.','а.','ов.');
  end;
end;
      ...
      begin
        ...
        begin
          try
            ...
            RemXForm.ShowWarning('Файл "'+ExtractFileName(FileName)+
                    '" занят другим процессом. Попробуйте еще раз.');
            Exit;
          end;
        end;
...
begin
  ...
  if Trim(FileName) = '' then
    ...
    if RemxForm.ShowQuestion(
        'Записать мнемосхему в файл "'+FileName+'"?')=mrOK then
    begin
...
begin
  ...
  try
    OpenExtDialogForm.Filter:='Все файлы '+
                            '(*.jpg;*.jpeg;*.bmp;*.ico;*.emf;*.wmf;*.png)|'+
                            '*.jpg;*.jpeg;*.bmp;*.ico;*.emf;*.wmf;*.png|'+
                            'Файлы JPEG (*.jpg)|*.jpg|'+
                            'Файлы JPEG (*.jpeg)|*.jpeg|'+
                            'Файлы BITMAP (*.bmp)|*.bmp|'+
                            'Файлы ICO (*.ico)|*.ico|'+
                            'Файлы METAFILE (*.emf)|*.emf|'+
                            'Файлы METAFILE (*.wmf)|*.wmf|'+
                            'Файлы PNG (*.png)|*.png';
...
begin
  if RemXForm.ShowQuestion('Удалить фоновую картинку?') = mrOk then
  begin
    Background.Clear;
    HasChanged:=True;
  end;
...
begin
  ...
  try
    SaveExtDialogForm.Filter:='Все файлы '+
                            '(*.jpg;*.jpeg;*.bmp;*.ico;*.emf;*.wmf;*.png)|'+
                            '*.jpg;*.jpeg;*.bmp;*.ico;*.emf;*.wmf;*.png|'+
                            'Файлы JPEG (*.jpg)|*.jpg|'+
                            'Файлы JPEG (*.jpeg)|*.jpeg|'+
    ...
    begin
      ...
      if not ((Pos('All',W1) = 1) or (Pos('Все',W1) = 1)) and
         (Pos(Ext,W2) > 0) then
      begin
...
begin
  if SchemeSave.Enabled and
    (RemxForm.ShowQuestion('Последние изменения не записаны!'+
               #13+'Записать изменения в файл?')=mrOK) then
      SchemeSave.Execute;
end;
...
begin
  SaveExtDialogForm:=TSaveExtDialogForm.Create(Self);
  try
    SaveExtDialogForm.Filter:='Мнемосхемы (*.SCM)|*.scm';
    SaveExtDialogForm.InitialDir:=Caddy.CurrentSchemsPath;
