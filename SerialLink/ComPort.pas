...
Type
 ...
 TComProp = class(TPersistent)
  ...
  Public
   ...
   function GetWaitTxDelay: Cardinal;
  Published
   {Скорость обмена}
 ...
 TComPort = class //(TComponent)
  Protected
    ...
    procedure SetReadActive(const Value: Boolean); virtual;
    function  GetReadActive: Boolean; virtual;
  ...
  Public
    ...
    Function  WriteString(const S : String) : Boolean; virtual;
...
Begin
 ...
 Try
   ...
   Sleep(2); // ВЕСЬМА необходимая задержка !!!
 Finally
...
//   Success :=
   EnumPorts(nil, 1, PortsPtr, BytesNeeded, BytesNeeded, Returned);
...
//   Success :=
   EnumPorts(nil, 2, PortsPtr, BytesNeeded, BytesNeeded, Returned);
...
Begin
 {Освобождаем буфер}
 FreeMem(FInBuffer, FOwner.FInBufSize);
 Inherited Destroy;
End;
      ...
      If GetOverlappedResult(FHandle, ReadOL, BytesTrans, False) then begin
       ...
       If (Mask and EV_RXCHAR) <> 0 then begin
        ...
        If bReadable then begin
         ...
         If ReadFile(FHandle, FInBuffer^, Min(FInBufSize, AvaibleBytes),
...
{значение паузы для полной передачи всех байт}
function TComProp.GetWaitTxDelay: Cardinal;
