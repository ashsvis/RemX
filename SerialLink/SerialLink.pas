...
var Buffer: TBuff; Len: Integer; N,LastError: Cardinal;
    WriteOL: TOverLapped; Signaled: Cardinal; Result: boolean;
begin
  if Text <> '' then
  begin
    if Handle <> INVALID_HANDLE_VALUE then
...
(*
      ...
      try
        WriteFile(Handle,Buffer,Len,N,@WriteOL);
        Signaled:=WaitForSingleObject(WriteOL.hEvent,INFINITE);
        Result:=
          (Signaled = WAIT_OBJECT_0) and
          (GetOVerlappedResult(Handle,WriteOL,N,False));
        FBuzy:=Result;
      finally
...
var N: Cardinal; TS: Integer;
    ReadOL: TOverlapped; Signaled, Mask: DWord; CurrentState: TComStat;
    ErrCode,AvaibleBytes: Cardinal; bReadable: boolean;
begin
(*
  try
//    Windows.Beep(2000,30);
    ...
    while (not Terminated) and (Handle <> INVALID_HANDLE_VALUE) do
    begin
      WaitCommEvent(Handle,Mask,@ReadOL);
      Signaled:=WaitForSingleObject(ReadOL.hEvent,INFINITE);
      if Signaled = WAIT_OBJECT_0 then
      begin
        if GetOverlappedResult(Handle,ReadOL,N,True) then
        begin
          if (Mask and EV_RXCHAR) <> 0 then
          begin
            ClearCommError(Handle,ErrCode,@CurrentState);
            AvaibleBytes:=CurrentState.cbInQue;
            bReadable:=(AvaibleBytes > 0);
            if bReadable then
            begin
              ZeroMemory(@Buff[0],SizeOf(Buff));
              if ReadFile(Handle,Buff,Min(SizeOf(Buff),AvaibleBytes),
                          N,@ReadOL) then
              begin
                Count:=N;
...
//        else
//          Count:=GetLastError and ERROR_IO_PENDING;
      end;
    end;
  finally
