...
uses
  ...
  Dialogs, EntityUnit, VirtualUnit;

type
  TVirtNNEditForm = class(TVirtEditForm)
  private
    E: TVirtNumeric;
  protected
...
var
  VirtNNEditForm: TVirtNNEditForm;

implementation

uses ComCtrls, StrUtils, Math, Menus, GetPtNameUnit;

{$R *.dfm}

{ TVirtNNEditForm }

procedure TVirtNNEditForm.ConnectEntity(Entity: TEntity);
var sFormat: string;
begin
  inherited;
...
procedure TVirtNNEditForm.PopupMenu1Popup(Sender: TObject);
...
procedure TVirtNNEditForm.ChangeTextClick(Sender: TObject);
...
begin
  ...
  begin
    if L.Caption = 'Размерность' then
      ...
      Exit;
    if InputStringDlg(L.Caption,S,9) then
    begin
      if L.Caption = 'Размерность' then
      begin
        E.EUDesc:=S;
...
procedure TVirtNNEditForm.ChangeBooleanClick(Sender: TObject);
...
procedure TVirtNNEditForm.ChangePVFormatClick(Sender: TObject);
...
procedure TVirtNNEditForm.ChangeFloatClick(Sender: TObject);
...
procedure TVirtNNEditForm.ChangeAlmDBClick(Sender: TObject);
var L: TListItem; M: TMenuItem; sFormat: string;
begin
  L:=ListView1.Selected;
  M:=Sender as TMenuItem;
