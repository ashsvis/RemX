...
type
  TVirtNNPaspForm = class(TForm,IEntity)
    Panel1: TPanel;
    PTNameDesc: TLabel;
    PTDesc: TLabel;
...
var
  VirtNNPaspForm: TVirtNNPaspForm;
...
procedure TVirtNNPaspForm.ConnectEntity(Entity: TEntity);
var sFormat: string; k: Double;
begin
  E:=Entity as TVirtNumeric;
...
procedure TVirtNNPaspForm.ConnectImageList(ImageList: TImageList);
begin
// Stub
end;

procedure TVirtNNPaspForm.UpdateRealTime;
var sFormat: string; k: TAlarmState;
begin
  ButtonToDataBase.Visible:=Caddy.UserLevel > 4;
  EnterButton.Visible:=(Caddy.UserLevel > 1) and not Assigned(E.SourceEntity);
  AlEnbSt.Caption:=IfThen(E.Logged,'Да','Нет');
  if E.RealTime > 0 then
    LastTime.Caption:=Format('%.3f',[E.RealTime/1000])+' сек'
  else
...
procedure TVirtNNPaspForm.FormCreate(Sender: TObject);
begin
...
procedure TVirtNNPaspForm.FormResize(Sender: TObject);
begin
  RxGraph.Left:=10;
...
procedure TVirtNNPaspForm.PVHHTPMouseDown(Sender: TObject;
...
procedure TVirtNNPaspForm.PVHHTPMouseEnter(Sender: TObject);
begin
...
procedure TVirtNNPaspForm.PVHHTPMouseLeave(Sender: TObject);
begin
  with Sender as TLabel do
    TShape(Tag).Pen.Color:=clBlack;
end;

procedure TVirtNNPaspForm.ButtonToDataBaseClick(Sender: TObject);
begin
  E.ShowEditor(Monitor.MonitorNum);
end;

procedure TVirtNNPaspForm.PVSourceDblClick(Sender: TObject);
begin
  E.ShowParentPassport(Monitor.MonitorNum);
end;

procedure TVirtNNPaspForm.ButtonToMainScreenClick(Sender: TObject);
begin
  E.ShowScheme(Monitor.MonitorNum);
end;

procedure TVirtNNPaspForm.EnterButtonClick(Sender: TObject);
