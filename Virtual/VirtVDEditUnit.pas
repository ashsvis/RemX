...
type
  TVirtVDEditForm = class(TBaseEditForm)
    ListView1: TListView;
    ListView2: TListView;
    Splitter1: TSplitter;
...
var
  VirtVDEditForm: TVirtVDEditForm;
...
procedure TVirtVDEditForm.BaseFresh(var Mess: TMessage);
begin
  UpdateRealTime;
end;

procedure TVirtVDEditForm.ConnectBaseEditor(var Mess: TMessage);
begin
  ConnectImageList(TImageList(Mess.WParam));
  ConnectEntity(TEntity(Mess.LParam));
end;

procedure TVirtVDEditForm.ConnectEntity(Entity: TEntity);
var i: integer;
begin
...
procedure TVirtVDEditForm.ConnectImageList(ImageList: TImageList);
begin
  ListView1.SmallImages:=ImageList;
  ListView2.SmallImages:=ImageList;
end;

procedure TVirtVDEditForm.ListView2DblClick(Sender: TObject);
var E: TEntity;
begin
...
procedure TVirtVDEditForm.ListView1DblClick(Sender: TObject);
begin
...
procedure TVirtVDEditForm.UpdateRealTime;
var L: TListItem;
begin
...
procedure TVirtVDEditForm.PopupMenu1Popup(Sender: TObject);
...
procedure TVirtVDEditForm.ChangeTextClick(Sender: TObject);
...
procedure TVirtVDEditForm.ChangeFetchClick(Sender: TObject);
...
procedure TVirtVDEditForm.ChangeBooleanClick(Sender: TObject);
...
procedure TVirtVDEditForm.PopupMenu2Popup(Sender: TObject);
...
procedure TVirtVDEditForm.ChangeLinkClick(Sender: TObject);
...
begin
  ...
  try
    ...
    begin
      ...
      while R <> nil do
      begin
        if not Assigned(R.SourceEntity) and R.IsDigit and
           not R.IsParam then
          List.Add(R);
        R:=R.NextEntity;
      end;
...
procedure TVirtVDEditForm.DeleteLinkClick(Sender: TObject);
