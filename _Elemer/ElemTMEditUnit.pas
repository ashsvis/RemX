...
uses
  ...
  Dialogs, EntityUnit, ElemerUnit;

type
  TElemTMEditForm = class(TElemEditForm)
  private
    E: TTMAnaOut;
  protected
...
var
  ElemTMEditForm: TElemTMEditForm;
...
procedure TElemTMEditForm.ConnectEntity(Entity: TEntity);
var sFormat: string;
begin
  inherited;
...
procedure TElemTMEditForm.PopupMenu1Popup(Sender: TObject);
...
begin
  ...
  begin
    if Assigned(L) then
    begin
      if (L.Caption = 'Номер канала прибора') then
      begin
        M:=TMenuItem.Create(Self);
...
procedure TElemTMEditForm.ChangeTextClick(Sender: TObject);
...
procedure TElemTMEditForm.ChangeAlmDBClick(Sender: TObject);
...
procedure TElemTMEditForm.ChangeBooleanClick(Sender: TObject);
...
procedure TElemTMEditForm.ChangeFloatClick(Sender: TObject);
...
procedure TElemTMEditForm.ChangePVFormatClick(Sender: TObject);
var L: TListItem; M: TMenuItem; sFormat: string;
begin
  L:=ListView1.Selected;
  M:=Sender as TMenuItem;
...
procedure TElemTMEditForm.ChangeIntegerClick(Sender: TObject);
...
begin
  ...
  if Assigned(L) then
  begin
    if L.Caption = 'Номер канала прибора' then V:=E.Place
    else
      Exit;
    ...
    begin
      if L.Caption = 'Номер канала прибора' then
      begin
        if InRange(V,1,8) then
        begin
          E.Place:=V;
          L.SubItems[0]:=Format('%d',[E.Place]);
        end
        else
          raise ERangeError.Create('Значение вне допустимых границ (1..8)!');
      end;
    end;
  end;
end;
