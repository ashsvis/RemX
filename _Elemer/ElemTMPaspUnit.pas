...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, Buttons, EntityUnit, ElemerUnit, RxGraph;

type
  TElemTMPaspForm = class(TForm,IEntity)
    Panel1: TPanel;
    PTNameDesc: TLabel;
    PTDesc: TLabel;
...
var
  ElemTMPaspForm: TElemTMPaspForm;
...
procedure TElemTMPaspForm.ConnectEntity(Entity: TEntity);
var sFormat: string; k: Double;
begin
  E:=Entity as TTMAnaOut;
...
//-------------------------------------
  ...
  Output.Caption:=Format('%d',[E.Place]);
  UpdateRealTime;
end;

procedure TElemTMPaspForm.ConnectImageList(ImageList: TImageList);
begin
// Stub
end;

procedure TElemTMPaspForm.UpdateRealTime;
...
procedure TElemTMPaspForm.FormCreate(Sender: TObject);
...
procedure TElemTMPaspForm.FormResize(Sender: TObject);
begin
  RxGraph.Left:=10;
...
procedure TElemTMPaspForm.PVHHTPMouseDown(Sender: TObject;
...
procedure TElemTMPaspForm.PVHHTPMouseEnter(Sender: TObject);
begin
...
procedure TElemTMPaspForm.PVHHTPMouseLeave(Sender: TObject);
begin
  with Sender as TLabel do
    TShape(Tag).Pen.Color:=clBlack;
end;

procedure TElemTMPaspForm.ButtonToDataBaseClick(Sender: TObject);
begin
  E.ShowEditor(Monitor.MonitorNum);
end;

procedure TElemTMPaspForm.PVSourceDblClick(Sender: TObject);
begin
  E.ShowParentPassport(Monitor.MonitorNum);
end;

procedure TElemTMPaspForm.ButtonToMainScreenClick(Sender: TObject);
begin
